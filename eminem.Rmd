...
registerDoParallel()
writeStats <- function() {
  write.csv(x=data.frame(datetime=start.time,
                         host=Sys.info()["nodename"],
                         replications=n.replications,
                         boxes=n.boxes,
                         candies=n.candies,
                         elapsed=as.numeric(Sys.time() - start.time),
                         cores),
            row.names=FALSE,
            append=TRUE,
...
\footnotetext[2]{Computation took `r  Sys.time() - start.time` minutes using `r getDoParWorkers()` cores.}
